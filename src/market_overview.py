"""
Market overview for the dashboard's Market Context panel.

Presentation of free/existing data: indices (SPY/QQQ/IWM) + VIX via yfinance, a
breadth proxy computed from the sector rankings we already store, an earnings
calendar for names currently held, and recent market news headlines. The heavy
bits (news, earnings) are cached so the dashboard poll stays cheap.

There used to be a static, hand-typed "economic calendar" (FOMC/CPI/jobs
dates) here. It was never actually pulled from an API -- just a fixed list
someone typed in once -- so it silently went stale and was removed. If a real
calendar feed is wired up later (an API we actually hold a key for), it can
come back; until then, don't fake it.
...
def build(alpaca, sector_rankings, held_symbols, regime=None) -> dict[str, Any]:
    return {
